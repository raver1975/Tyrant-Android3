...
public class Combat {
	...
	public static int attack(Thing a, Thing target) {
		if (Game.hero().equals(a))Game.hero().isRunning(false);
		if (target == null)
			return 0;
	...
	public static int attackWith(Thing a, Thing target, Thing w) {
		...
		if ((map != null)&&a.isHero()) {
			if (target.getFlag("IsDoor"))map.areaNotify(a.x, a.y, 4, AI.EVENT_ATTACKED, target
	...
	private static void kickEffect(Thing b, Thing t) {
		...
		//System.out.println(b.name()+" attack "+t.name()+" with "+kickWeapon.name());
