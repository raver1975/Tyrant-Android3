...
public class Movement {
	...
	public static boolean tryMove(Thing thing, Map map, int tx, int ty) {
		...
		if (map == null) {
			System.out.println("no map in Movement.tryMove ["
					+ thing.getName(Game.hero()) + "]");
			return false;
		}
	...
	private static boolean tryDisplace(Thing t, Thing m) {
		...
		if (success) {
			...
			if (Tile.isSensibleMove(t, map, tx, ty)
					&& Tile.isSensibleMove(m, map, fx, fy)) {
				t.moveTo(map, tx, ty);
				m.moveTo(map, fx, fy);
	 ...
	 */
	private static void enterTrigger(Thing t, Map m, int tx, int ty,
			boolean touchFloor) {
	...
	private static void locationMessage() {
		...
		while (t != null) {
			if (t.getFlag("IsItem")) {
				...
				if (t.getFlag("IsShopOwned")) {
					Thing sk = Item.findShopkeeper(t);
					int val = 0;
					if (sk != null)
						val = Item.shopPrice(t, h, sk);
					s = s + " (" + Coin.valueString(val) + ") for sale";
				}
				Game.message("There "
	...
	private static boolean tryRunningMove(Thing thing, Map map, int tx, int ty,
			int dx, int dy) {
		boolean canMove = canMove(thing, map, tx, ty);
		int previousExits = -1;
		int runCount = thing.getStat("RunCount");
		if (runCount != 1)
			canMove = !interruptsRunning(thing, map, tx, ty);
		if (canMove) {
			previousExits = thing.orthogonalExits(dx, dy).size();
			doMove(thing, map, tx, ty);
			if (runCount == 1)
				return true;
			boolean isStillRunning = previousExits == thing.orthogonalExits(dx,
					dy).size();
		...
		for (Iterator iter = freedoms.iterator(); iter.hasNext();) {
			...
			if (map.getPath(placeToMove.x, placeToMove.y) == 1)
				continue;
			if (interruptsRunning(thing, map, placeToMove.x, placeToMove.y))
				continue;
			int oldX = thing.x;
			int oldY = thing.y;
	...
	private static boolean interruptsRunning(Thing thing, Map map, int tx,
			int ty) {
		if (map.isBlocked(tx, ty))
			return true;
		Thing next;
		for (next = map.getObjects(tx, ty); next != null; next = next.next) {
			if (thing.isDoorVisible(tx, ty))
				return true;
			if (next.get("Message") != null)
				continue;
			if (next.getFlag("IsMarkerPortal"))
				continue;
			if (next.getFlag("IsMarker"))
				continue;
			return true;
		}
		return false;
