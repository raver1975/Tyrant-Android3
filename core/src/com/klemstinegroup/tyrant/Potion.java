...
/**
 * 
 * A potion represents a magical liquid which can be drunk with various effects.
 * Some may be good, some may be bad so drinking an unidentified Potion may be
 * dangerous.....
 * 
 * TODO: make all potion effects into spells and change Potion so that it is in
 * effect casting the spell on the drinker. This will also allow potion effects
 * to be re-used as spells or special effects for other items.
 */
public class Potion {
	...
	private static String[] potioncolours = { "red", "burgundy", "orange",
			...
			"black", "rainbow" };
	private static int[] images = { 245, 245, 240, 240, 247, 246, 248, 248,
			248, 243, 244, 244, 244, 250, 243, 242, 241, 251 };
	...
	public static void addPotion(Thing t) {
		...
		t.set("UName", potionstyles[style] + " " + potioncolours[colour]
				+ " potion");
	...
	public static void dip(Thing user, Thing target, Thing potion) {
		...
		else Game.message("Nothing happens.");
	}
	...
	private static class DipScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			Game.selectItem("Select an item to apply the potion to:",
					user.getItems());
			Engine.instance.input = '3';
			Engine.instance.inputThing=t;
			return true;
		}
	}
	...
	public static class GainScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			if ((max > 0) && (targ.getBaseStat(stat) >= max))
				return false;
	...
	public static void initRandomPotions() {
		...
		{
			Thing effect = Effect.temporary(Modifier.bonus("MoveSpeed", 100),
					2000);
		...
		// create bonus effect
		{
			Thing effect = Effect.temporary(Modifier.bonus("ST", RPG.d(2, 6)),
					2000);
		...
		// create bonus effect
		{
			Thing effect = Effect.temporary(Modifier.bonus("IN", RPG.d(3, 6)),
					2000);
		...
		// create bonus effect
		{
			Thing effect = Effect.temporary(Modifier.bonus("CH", RPG.d(2, 8)),
					2000);
		...
		// create bonus effect
		{
			Thing effect = Effect.temporary(
					Modifier.bonus("AttackSpeed", RPG.d(2, 30)), 2000);
		...
		{
			Thing effect = Effect.temporary(
					Modifier.linear("IsImmortal", 0, 1), 1000);
		...
		// create bonus effect
		{
			Thing effect = Effect.temporary(Modifier.linear("IsInsane", 0, 1),
					30000);
		...
		t.addHandler("OnDip", new Script() {
			...
			public boolean handle(Thing t, Event e) {
				...
				if (t.getFlag("IsCursed")) {
					Game.message("The potion seems to damage "
							+ tt.getYourName());
					Damage.inflict(tt, tt.getStat("HPSMAX") / 2, RPG.DT_SPECIAL);
				} else {
					if (Item.repair(tt, false)) {
						Game.message(tt.getYourName()
								+ " is surrounded by a shining orange light");
					} else {
						Game.message(tt.getYourName()
								+ " glows orange for a second");
					}
				}
				return false;
			}
		...
		t.set("LevelMin", 1);
		t.set("OnDrunk",
				Scripts.statGain("Target", "SK,ST,AG,TG,IN,WP,CH,CR", 12));
		addPotion(t);

		t = Lib.extend("potion of talent", "base potion");
		t.set("LevelMin", 10);
		t.set("OnDrunk",
				Scripts.statGain("Target", "SK,ST,AG,TG,IN,WP,CH,CR", 30));
		addPotion(t);

		t = Lib.extend("potion of greater talent", "base potion");
		t.set("LevelMin", 20);
		t.set("OnDrunk",
				Scripts.statGain("Target", "SK,ST,AG,TG,IN,WP,CH,CR", 50));
		addPotion(t);

		t = Lib.extend("potion of supreme talent", "base potion");
		t.set("LevelMin", 32);
		t.set("OnDrunk",
				Scripts.statGain("Target", "SK,ST,AG,TG,IN,WP,CH,CR", 90));
		addPotion(t);

		t = Lib.extend("potion of legendary talent", "base potion");
		t.set("LevelMin", 42);
		t.set("OnDrunk",
				Scripts.statGain("Target", "SK,ST,AG,TG,IN,WP,CH,CR", 170));
		addPotion(t);
