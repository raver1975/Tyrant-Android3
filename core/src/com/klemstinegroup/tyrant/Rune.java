...
public class Rune {
	...
	public static int countExistingRunes(Thing t) {
	...
	public static int destroyExistingRunes(Thing t, int chance) {
		Thing[] rs=t.getFlaggedContents("IsRune");
		int rc=0;
		for (int i=0; i<rs.length; i++) {
			if (RPG.r(100)<chance) {
	...
	private static class RuneScribeScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
    		...
    		Game.selectString("Select a rune to scribe: ",rs,true);
    		Engine.instance.input = '5';
	...
	private static class RuneUseScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
    		Thing user=e.getThing("User");
    		Game.selectItem("Select an item to apply the rune to:",user.getItems());
			Engine.instance.input = '4';
			Engine.instance.inputThing=t;
    		return true;
		}
	}
