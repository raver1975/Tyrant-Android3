...
public class Person {
					...
					if (Coin.getMoney(h) < price) {
						...
						if (c == 'y') {
							...
							Game.message("You learn the " + sk + " skill from "
									+ t.getTheName()+"for " + Coin.valueString(price)+ "?");
						} else {
	...
	private static class ListSellingScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			Item.tryIdentify(t,things);
			Engine.instance.inputThing = t;
			Engine.instance.input = '0';
			Engine.inputThings=things;
			Game.selectSale("Select an item to sell:", things);
			Engine.instance.input = '0';
			Engine.instance.inputThing = t;
			return true;
		}
	...
	private static class TeachingScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			//List<String> newList = new ArrayList<String>(sks);
			//newList.addAll(sksr);
			Game.selectString("Select a skill to improve:", (String[]) sks.toArray(new String[0]),true);
			Engine.input='S';
			return true;
		}
	...
	private static class RiddleScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			responder.incStat(Text.camelizeString(t.getName())
					+ "RiddleSolved", 1);
			/*
