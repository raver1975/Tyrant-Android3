...
public class Recipe {
	...
	public static boolean apply(Thing h, String s) {
		...
		//System.out.println("Recipe.apply");
		//ListScreen ls=new ListScreen("Select your recipe:",al);
		//Component gs=QuestApp.getInstance().getScreen();
		//QuestApp.getInstance().switchScreen(ls);
		//String rec=(String)ls.getObject();
		//QuestApp.getInstance().switchScreen(gs);
		Game.selectString("Select your recipe:", (String[])al.toArray(new String[0]), true);
		Engine.input='A';
		Engine.inputThings=(Thing[])al1.toArray(new Thing[0]);

		return false;
	}
	
	public static Thing recipeResult(Thing r) {
