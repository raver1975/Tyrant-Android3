...
public class GameHandler {
	...
	public void doDirection(Thing thing, Action action, boolean isShiftDown) {
		...
		if (map == null || thing == null || action == null)
			return;
		// handle movement commands
		if (map.getTile(thing.x + direction.x, thing.y + direction.y) != 0) {
			if (isShiftDown) {
				if (thing.getStat("DirectionX") != direction.x
						|| thing.getStat("DirectionY") != direction.y
						|| !thing.isRunning()) {
					/*
					 * This is needed for running around corners. The DirectionX
					 * changes when you run around a corner but the key sent
					 * doesn't so direction.x is stale. Only clear the path when
			...
			boolean moved = Movement.tryMove(thing, map, thing.x + direction.x,
					thing.y + direction.y);
			if (!moved) {
			...
			char c = 'y';
			if ((thing.x == 0) && ("147".indexOf(c) >= 0))
				c = 'y';
			if ((thing.y == 0) && ("789".indexOf(c) >= 0))
				c = 'y';
			if ((thing.x == (map.getWidth() - 1)) && ("369".indexOf(c) >= 0))
				c = 'y';
			if ((thing.y == (map.getWidth() - 1)) && ("123".indexOf(c) >= 0))
				c = 'y';
			c = 'y';
			if (c == 'y') {
				Game.message("Exiting area, please wait...");
