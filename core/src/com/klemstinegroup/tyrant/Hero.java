 ...
 * 
 * Note that experience is currently only applicable to the hero, but this may
...
public class Hero {
	...
	public static Map worldMap() {
		if (Game.hero()!=null)return (Map) Game.hero().get("WorldMap");
		return null;
	}

	/**
	 * This function awards some bonus items based on the hero's professional
	 * skills.
	 * 
	 * @param h
	 */
	private static void applyBonusItems(Thing h) {
		...
		if (h.getFlag(Skill.ARCHERY) ) {
			...
			Thing t = Lib.createType("IsRangedWeapon",
					RPG.d(h.getStat(Skill.ARCHERY), 6));
			Thing ms = RangedWeapon.createAmmo(t,
					RPG.d(h.getStat(Skill.ARCHERY), 6));
			h.addThing(t);
			h.addThing(ms);
		}
	...
	private static void applyProfession(Thing h, String p) {
		...
		if (p.equals("fighter")) {
			...
			for (int i = 0; i < 10; i++)
				h.addThingWithStacking(Lib.createType("IsHerb", RPG.d(10)));
			for (int i = 0; i < 6; i++)
				h.addThingWithStacking(Lib.createType("IsMonsterPart",
						RPG.d(10)));
	...
	public static void gainExperience(int x) {
		Thing h = Game.hero();
		Game.warn("You gain "+x+" experience points");
	...
	/**
	 * Calculate the number of experience points required to gain a particular
	 * level
	 * 
	 * @param level
	 *            Target level
	...
	public static String reportKillData() {
		...
		for (Iterator it = al.iterator(); it.hasNext();) {
			...
			String g = Text.centrePad(count.toString(),"",  4);
			g=" "+g+name;
			sb.append(g + "\n");
		}
	...
	private static String[] hungerDecayStats = new String[] { "SK", "ST", "AG",
			"TG", "IN", "WP", "CH", "CR" };

	// hero names used for search
	private static final String[] races = { "human", "hobbit", "dwarf",
			"high elf", "wood elf", "dark elf", "gnome", "half orc",
	...
	public static String[] heroRaceDescriptions() {
		...
		for (int i = 0; i < descs.length; i++) {
			...
			if (d == null)
				d = "No description for " + races[i] + " yet...";
			descs[i] = d;
		}
		return descs;
	}
	...
	public static String[] heroProfessionDescriptions(String race) {
		...
		pds.put("priest",
				"Priests dedicate their lives to the service of their God. They seek to build temples, help the needy and convert followers to the glory of God, and are always ready to take up arms to strike down blasphemers and heretics. As priests gain power, they are often granted divine aid to aid them in their quests. To serve their God is the only reward they need.");
		pds.put("thief",
	...
	private static String[] months = { null, "Windalmar", "Ferrentir",
			"Antalyir", "Foolsdawn", "Whispertide", "Evermoor", "Brightsun",
			"Kingsmoon", "Harvestmoon", "Eberfest", "Witherfrost", "Hogmandark" };
	...
	public static void createHeroHistory(Thing h) {
		...
		sb.append("You are born " + (Text.isVowel(r.charAt(0)) ? "an" : "a")
				+ " " + r + " on the " + dayString + " of " + monthString
				+ ". ");
		...
		case 1:
			...
			break;
		case 2:
			sb.append("You were extremely devout. It caused you great anguish because you never felt that "
					+ god + " was truly close to you. ");
			h.incStat("WP", 1);
			h.incStat("CH", -1);

			break;
		case 3:
			sb.append("You were extremely devout, and your priest praised you for having earnt the blessing of "
					+ god + ". ");
			h.incStat("Luck", 4);
			h.incStat(Skill.PRAYER, 1);
			break;
		default:
			sb.append("You were not particularly devout, but still felt that "
		...
		case 2:
			...
			h.incStat(
					RPG.pick(new String[] { Skill.SMITHING, Skill.PAINTING,
							Skill.MUSIC, Skill.COOKING }), 1);
			h.incStat("ST", -1);
			h.incStat("IN", -1);
		case 3:
		...
		sb.append("Determined to make something of your life, you began your "
				+ p + " training as soon as you were old enough. ");
		switch (RPG.d(5)) {
		default:
			sb.append("You showed a good aptitude for your chosen career, and before too long your tutor proclaimed you as a fully trained "
					+ p + ".");
			break;
		}
		sb.append("\n\n");
	...
	public static void action(Thing h, int t) {
		...
		hunger = RPG.min(hungerThreshold * 3,
				hunger + (t * 6) / (6 + h.getStat(Skill.SURVIVAL)));
		...
		case 2:
			for (int i = RPG.po(t, 10000); i > 0; i--) {
				...
				if (!h.getFlag("IsImmortal"))
					h.set(stat, RPG.max(sv - 1, 1));
			}
			break;
		case 3:
			// dying of hunger
			...
			if (!h.getFlag("IsImmortal"))
				h.incStat("HPSMAX", -loss);
			if (!h.getFlag("IsImmortal"))
				h.incStat("HPS", -loss * 2);
			break;
		}
		...
		// thief searches
		for (int i = RPG.po(
				t * h.getStat(Skill.ALERTNESS) * h.getStat(RPG.ST_CR), 10000); i > 0; i--) {
			Secret.search();
		}
	...
	public static boolean hasHungerString(Thing h) {
		...
		int hp = (threshold > 0) ? (h.getStat(RPG.ST_HUNGER) * 100) / threshold
				: 0;
