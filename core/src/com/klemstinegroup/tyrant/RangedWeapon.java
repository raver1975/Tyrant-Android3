...
public class RangedWeapon {
	...
	public static void useRangedWeapon(Thing w, Thing user) {
		if (user.isHero()) {
			...
			if (!isValidAmmo(w, missile)) {
				...
				if (ms.length > 1) {
					missile = Game.selectItem(
							"Select ammunition for your "
			...
			if (missile != null) {
				...
				Game.message("You have no ammunition for your "
						+ w.getName(Game.hero()));
			}
		}
	}
	...
	public static void fire(Thing rangedWeapon, Thing shooter, Thing missile) {
		...
		if (shooter == h) {
			if (isValidAmmo(rangedWeapon, missile)) {
				Point p = Engine.instance.getTargetLocation(Engine.instance.map.findNearestFoe(h));
				// Do not confuse player with possible false info
				Game.message("");
				if (p != null) {
					missile = missile.remove(1);
					fireAt(rangedWeapon, shooter, missile, Engine.instance.map, p.x, p.y);
				}
	...
	public static void fireAt(Thing rangedWeapon, Thing shooter, Thing missile,
			Map m, int tx, int ty) {
		if (missile != null) {
			...
			if (target == null) {
				...
				// have scored a hit!
				int rst = (shooter.getStat(RPG.ST_ST) * (4 + shooter
						.getStat(Skill.ARCHERY))) / 4;
				...
				if (target.isVisible(Game.hero())) {
					if (shooter.isHero()) {
						...
						case 1:
							...
							break;
						case 2:
							Game.message("You fire and score a great hit on "
									+ target.getTheName());
							break;
						default:
							Game.message("You fire and score a perfect hit on "
									+ target.getTheName());
							break;
						}
					} else {
