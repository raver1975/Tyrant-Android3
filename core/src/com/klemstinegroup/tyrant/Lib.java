...
import com.klemstinegroup.tyrantandroid.Renderer;
...
public class Lib extends Object implements Serializable, Cloneable {
	...
	public List getTypeArray(String flag, int level) {
        ...
        List temp1=(List)types.get(flag);
        if (temp1==null)return Collections.EMPTY_LIST;
		for (int i = 0; i < temp1.size(); i++) {
			Thing thing = new Thing((BaseObject) temp1.get(i));
			if (!isBaseClass(thing) && thing.getFlag(flag)) {
	...
	public static Thing extend(String newName, String baseName) {
		...
		if (baseObject == null)return Lib.create("Wish");
//			throw new Error("Can't find base properties [" + baseName + "]");
	...
	private void addThingToTypeArray(Thing thing) {
		...
		for (int i = 0; i < ifs.length; i++) {
			...
			List levels = (List) types.get(ifAttribute);
			if (levels == null) {
	...
	public static void init() {
		// set up base classes
		initBase();
		Renderer.percent=1/27f;
		// create effects, spells and utility objects
		Special.init();
		Renderer.percent=2/27f;;
		Effect.init();
		Renderer.percent=3/27f;
		Skill.init();
		Renderer.percent=4/27f;
		Spell.init();
		Renderer.percent=5/27f;
		// call inidvidual library generation classes
		Renderer.percent=6/27f;
		Aura.init();
		Renderer.percent=7/27f;
		Being.init();
		Renderer.percent=8/27f;
		Special.initClouds();
		Renderer.percent=9/27f;
		Item.init();
		Renderer.percent=10/27f;
		Decoration.init();
		Renderer.percent=11/27f;
		Scenery.init();
		Renderer.percent=12/27f;
		Tile.init();
		Renderer.percent=13/27f;
		Door.init();
		Tutorial.init();
		Renderer.percent=14/27f;
		Chest.init();
		Renderer.percent=15/27f;
		Trap.init();
		Renderer.percent=16/27f;
		Gods.init();
		Renderer.percent=17/27f;
		Quest.init();
		Renderer.percent=18/27f;
		Portal.init();
		Renderer.percent=19/27f;

		// init people and monsters
		AI.init();
		Renderer.percent=20/27f;
		Animal.init();
		Renderer.percent=21/27f;
		Monster.init();
		Renderer.percent=22/27f;
		Person.init();
		EndGame.init();
		Renderer.percent=23/27f;
		Artifact.init();
		Renderer.percent=24/27f;

		// inits that use random ingredients
		Recipe.init();
		Renderer.percent=25/27f;
		Spell.updateIngredients();
		Temple.init();
		Renderer.percent=26/27f;
